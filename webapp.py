...
from timetableAPI import fetch_timetable
...
def event(event):
    ...
    if 'username' in session:  #Checks if the user has a session if not then they can't access the homepage
        search = Searchbar() #Creates the Searchbar Object containing the form
        return render_template('events.html',posts=posts,search=search,event=event,timetable = fetch_timetable('maxoutin')) #Loads calender.html - which is the calendar page
